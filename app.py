...
with st.sidebar:
    ...
    btn=st.button("ANALİZİ BAŞLAT")  # buton beyaz olarak yukarıdaki CSS ile ayarlandı
...
if st.session_state['df_sonuc'] is not None:
    ...
    try:
        ...
        for i,(idx,row) in enumerate(df_son.head(4).iterrows()):
            with cols[i%4]:
                ...
                st.metric(label=f"{row['Taraf']}", value=f"{val_fmt}", delta=f"%{delta_pct:.1f}", key=f"kpi_{i}_{row['Kalem']}_{row['Taraf']}")
    except: pass
    ...
    # TAB1 TREND
    with tab1:
        kalem_sec=st.selectbox("Grafik Kalemi:", df["Kalem"].unique(), key="trend_kalem")
        ...
        fig=px.line(df_chart, x="Dönem", y="Değer", color="Taraf", title=f"📅 {kalem_sec} Trendi",
                    markers=True, category_orders={"Dönem": sirali_donemler}, color_discrete_sequence=["#FCB131","#000000","#555555"])
        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", hovermode="x unified")
        fig.update_yaxes(tickformat=",")
        st.plotly_chart(fig, use_container_width=True, key=f"trend_chart_{kalem_sec}")
    ...
    with tab2:
        ...
        with c_sim2:
            ...
            if not base_row.empty:
                ...
                with col_b: st.metric(f"Senaryo (%{artis_orani})", f"{yeni_deger:,.0f}".replace(",", "."), delta=f"{fark:,.0f}".replace(",", "."), key=f"sim_yeni_{kalem_sim}")
                sim_data=pd.DataFrame({"Durum":["Mevcut","Simülasyon"], "Tutar":[mevcut_deger,yeni_deger]})
                fig_sim=px.bar(sim_data, x="Durum", y="Tutar", color="Durum", text_auto='.2s', color_discrete_map={"Mevcut":"#000000","Simülasyon":"#FCB131"})
                fig_sim.update_layout(height=300, showlegend=False)
                st.plotly_chart(fig_sim,use_container_width=True,key=f"sim_chart_{kalem_sim}_{taraf_sim}")

    # TAB3 PAZAR PAYI
    with tab3:
        st.markdown("#### 📊 Pazar Payı Analizi")
        kalem_pie = st.selectbox("Payı İncelenecek Kalem:", df["Kalem"].unique(), key="pie_kalem")
        df_last = df[(df["TarihObj"] == df["TarihObj"].max()) & (df["Kalem"] == kalem_pie)]
        if "Sektör" in df_last["Taraf"].unique() and len(df_last["Taraf"].unique()) > 1:
                ...
                for idx, (i, r) in enumerate(other_rows.iterrows()):
                    ...
                    with cols_g[idx]:
                        fig_g = go.Figure(go.Indicator(
                            mode="gauge+number",
                            value=share_pct,
                            title={'text': f"{r['Taraf']} Payı"},
                            gauge={'axis': {'range': [0,100]}, 'bar': {'color': "#FCB131"}}
                        ))
                        fig_g.update_layout(height=300, margin=dict(l=20,r=20,t=50,b=20))
                        st.plotly_chart(fig_g, use_container_width=True, key=f"gauge_{r['Taraf']}_{kalem_pie}")
            else:
                st.warning("Sektör verisi yok.")
        else:
            fig_pie = px.pie(df_last, values="Değer", names="Taraf", hole=0.4,
                             color_discrete_sequence=["#FCB131","#000000","#333333"])
            st.plotly_chart(fig_pie, use_container_width=True, key=f"pie_simple_{kalem_pie}")
    ...
    with tab4:
        ...
        st.dataframe(display_df, use_container_width=True, height=400, key="detayli_tab")
        st.markdown("---")
        ...
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            ...
            for k in df["Kalem"].unique():
                ...
                df[df["Kalem"]==k].pivot(index="Dönem", columns="Taraf", values="Değer").to_excel(writer, sheet_name=safe_name)
        with open(buffer, "rb") as f:
            st.download_button("📥 EXCEL RAPORUNU İNDİR", f, "Vakif_Analiz.xlsx", key="excel_download")
